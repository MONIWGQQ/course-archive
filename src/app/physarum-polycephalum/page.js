...
const OrganicBackground = () => {
  ...
  useEffect(() => {
    ...
    if (lightParticlesRef.current.length === 0) {
      ...
      // 随机选取部分连线
      const pickCount = Math.min(14, edgeIndices.length); // 14个光点
      for (let k = 0; k < pickCount; k++) {
        ...
        lightParticlesRef.current.push({
          edge: [i, j, mx, my],
          t: Math.random(),
          dir: Math.random() > 0.5 ? 1 : -1, // 1:正向, -1:反向
          speed: 0.003 + Math.random() * 0.002 // 慢速
        });
      }
    }
    ...
    if (myceliumStructuresRef.current.length === 0) {
      const centerCount = 7 + Math.floor(Math.random() * 3); // 7~9个中心点
      const mainCount = 18 + Math.floor(Math.random() * 10); // 18~28条射线
      ...
      for (let c = 0; c < centerCount; c++) {
        ...
        centerY = canvas.height * (0.18 + Math.random() * 0.64); // 上下分布
        const lines = [];
      ...
      // 上下左右四个方向各加一个只显示爆炸光晕的中心
      const glowCenters = [
        { x: canvas.width / 2, y: canvas.height * 0.08 }, // 上
        { x: canvas.width / 2, y: canvas.height * 0.92 }, // 下
        { x: canvas.width * 0.08, y: canvas.height / 2 }, // 左
        { x: canvas.width * 0.92, y: canvas.height / 2 }, // 右
      ];
    ...
    const textBoxHeight = 32 + 36 + 32 + 36 + 120; // 估算高度（可根据实际内容微调）
    ...
    // 四个cell全部分布在文本框左侧，纵向排列
    const sideGap = 48; // 细胞距离文本框的水平距离
    const leftX = textBoxX - navRadius - sideGap;
    const offsetY = 60; // 整体下移
    const spread = textBoxHeight * 1.6; // 拉大分布区间
